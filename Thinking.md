...
|思考模式开关(1)|`{"thinking": {"type": "enabled/disabled"}}`|   |`{"reasoning": {"effort": "none/low/high/max"}}`  <br>(none 表示关闭思考模式)|
...
|   |   |   |
|---|---|---|
|请求传入 effort|deepseek-v4-flash  <br>实际映射 effort|deepseek-v4-pro  <br>实际映射 effort(3)|
|low|low|high|
|high|high|high|
|xhigh|high|max|
|max|max|max|
...
- **输出：** 模型先产生 **思维链1**（蓝色框，代表内部推理过程），最终输出 **回答1**。
    

**2. 第二轮对话**

- **输入（包含历史上下文）：** 包含了上一轮的所有内容 → **问题1 + 回答1 + 新的问题2**。
    
- **输出：** 模型基于上述完整输入，产生 **思维链2**，最终输出 **回答2**。
    

**3. 第三轮对话**

- **输入（历史继续累加）：** 包含了前两轮的完整互动 → **问题1 + 回答1 + 问题2 + 回答2 + 新的问题3**。
    
- **输出：** 模型基于累积的上下文，产生 **思维链3**，最终输出 **回答3**。
...
- **输入**：包含**工具**（可用能力）和**问题 1**（初始任务）。
    
- **输出**：模型进行第一次思考生成**思维链 1.1**，并据此决定执行 **工具调用 1.1**。
    

### 2. 请求 1.2（第一轮迭代）

- **输入**（整合了上一轮的输出）：**工具**、**问题 1**、**思维链 1.1**、**工具调用 1.1**、**调用结果 1.1**（即上一轮工具执行后的反馈）。
    
- **输出**：模型基于新的信息生成 **思维链 1.2**，并进行下一次 **工具调用 1.2**。
    

### 3. 请求 1.3（第二轮迭代与输出）

- **输入**（进一步累积历史记录）：**工具**、**问题 1**、**思维链 1.1**、**工具调用 1.1**、**调用结果 1.1**、**思维链 1.2**、**工具调用 1.2**、**调用结果 1.2**。
    
- **输出**：模型生成 **思维链 1.3**，并根据所有信息给出针对问题 1 的最终 **回答 1**。
    

### 4. 请求 2.1（上下文延展 / 新问题的开启）

- **输入**：此时输入变成了一个包含完整推理历史的长上下文。包括：**工具**、**问题 1**、**思维链 1.1**、**工具调用 1.1**、**调用结果 1.1**、**思维链 1.2**、**工具调用 1.2**、**调用结果 1.2**、**思维链 1.3**、**回答 1**，以及**新的问题 2**（说明模型或用户在回答 1 之后，提出了新的问题）。
    
- **输出**：生成针对新任务的 **思维链 2.1**，并进入无限循环（由“...”代表继续执行工具调用等流程）。
