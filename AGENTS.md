...
4. **Inline 自动补全**
   ...
   - 触发时机：停止输入 300ms 后或特定快捷键。
   - 补全建议基于当前文件、最近编辑历史与项目索引。
...
1. 定义插件清单与 API，实现插件发现、加载、隔离运行。
2. 让插件能注册命令、面板与 Agent 工具。
3. 实现 Agent 任务规划与多轮自动执行。
4. 实现 Inline 幽灵文本补全与下一行预测。
