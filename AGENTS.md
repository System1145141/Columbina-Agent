...
- **跨轮历史上下文（块索引 + recall）**：基于「Reordering Context System」最小实验验证（正确召回率 67% 且 0 误触发）落地。AI 面板同一会话内，更早轮次压缩为「固定区索引」（用户问题 + 助手回复一句话摘要，零 LLM 成本），最近一轮保留完整内容（待定区）；模型需要旧轮次细节时输出 `[recall:b轮次号]`，系统注入对应轮次完整内容后重新回答（最多 2 次）。任务规划的各执行步骤之间也自动共享该历史。
