...
interface GeneralSettings {
  ...
  language: "zh-CN";
  uiTheme: "deep-blue" | "light-blue" | "pearl-white";
}
...
declare global {
  interface Window {
    ...
    columbinaScheduler?: SchedulerApi;
    user?: UserApi;
...
const columbinaPanel = document.getElementById("columbina-panel") as HTMLFormElement;
...
const columbinaSaveStatus = document.getElementById("columbina-save-status") as HTMLElement;
...
const modelPresetSelect = document.getElementById("model-preset-select") as HTMLSelectElement;
const modelNicknameInput = document.getElementById("model-nickname") as HTMLInputElement;
const modelApikeyInput = document.getElementById("model-apikey") as HTMLInputElement;
const modelBaseurlInput = document.getElementById("model-baseurl") as HTMLInputElement;
const modelTransportSelect = document.getElementById("model-transport") as HTMLSelectElement;
const modelNameInput = document.getElementById("model-name") as HTMLInputElement;
const modelCancelBtn = document.getElementById("model-cancel-btn") as HTMLButtonElement;
const modelSaveBtn = document.getElementById("model-save-btn") as HTMLButtonElement;

let editingModelId: string | null = null;

function fillModelPresetOptions(): void {
  ...
  for (const preset of MODEL_PRESETS) {
    ...
    opt.value = preset.providerName;
    opt.textContent = preset.providerName;
    modelPresetSelect.appendChild(opt);
  }
}

function getModelPreset(providerName: string): ModelPreset | undefined {
  return MODEL_PRESETS.find((p) => p.providerName === providerName);
}

function onModelPresetChanged(): void {
  const providerName = modelPresetSelect.value;
  if (!providerName) return;
  const preset = getModelPreset(providerName);
  if (!preset) return;
  // 自动填充预设值
  if (!modelNicknameInput.value) modelNicknameInput.value = preset.shortName;
  if (!modelBaseurlInput.value) modelBaseurlInput.value = preset.baseUrl;
  if (preset.mainModels[0] && !modelNameInput.value) modelNameInput.value = preset.mainModels[0];
}

function renderModelList(): void {
  const items = modelEntries.map((entry) => {
    const div = document.createElement("div");
    div.className = "model-entry";
    div.dataset.modelId = entry.id;

    const info = document.createElement("div");
    info.className = "model-entry__info";

    const nick = document.createElement("span");
    nick.className = "model-entry__nickname";
    nick.textContent = entry.nickname || entry.model;
    info.appendChild(nick);

    if (entry.provider) {
      const prov = document.createElement("span");
      prov.className = "model-entry__provider";
  ...
  modelListEl.replaceChildren(...items);
}

function openModelDialog(editId?: string): void {
  editingModelId = editId ?? null;
  // 清空表单
  modelPresetSelect.value = "";
  modelNicknameInput.value = "";
  modelApikeyInput.value = "";
  modelBaseurlInput.value = "";
  modelTransportSelect.value = "auto";
  modelNameInput.value = "";

  if (editId) {
    const entry = modelEntries.find((m) => m.id === editId);
    if (entry) {
      // 找 preset 填充下拉
      if (getModelPreset(entry.provider)) {
        modelPresetSelect.value = entry.provider;
      }
      modelNicknameInput.value = entry.nickname;
      modelApikeyInput.value = entry.apiKey;
      modelBaseurlInput.value = entry.baseUrl;
      modelTransportSelect.value = entry.transport ?? "auto";
      modelNameInput.value = entry.model;
    }
  }
...
function saveModelFromDialog(): void {
  const nickname = modelNicknameInput.value.trim();
  const apiKey = modelApikeyInput.value.trim();
  const baseUrl = modelBaseurlInput.value.trim();
  const transport = modelTransportSelect.value as "auto" | "openai" | "anthropic";
  ...
  const provider = modelPresetSelect.value;

  if (editingModelId) {
    // 编辑已有
    const idx = modelEntries.findIndex((m) => m.id === editingModelId);
    if (idx !== -1) {
      modelEntries[idx] = {
        ...modelEntries[idx],
        nickname,
...
modelCancelBtn.addEventListener("click", closeModelDialog);
modelSaveBtn.addEventListener("click", saveModelFromDialog);
modelPresetSelect.addEventListener("change", onModelPresetChanged);
...
const NAV_LABELS: Record<string, { emoji: string; title: string; hint: string }> = {
  ...
  identity: { emoji: "💼", title: "职位", hint: "自定义 Columbina 的身份定位与工作职责" },
...
function setColumbinaSaveStatus(text: string, cls?: string): void {
  columbinaSaveStatus.textContent = text;
  columbinaSaveStatus.className = "save-status";
  if (cls) columbinaSaveStatus.classList.add(cls);
}
...
async function toggleSchedulerHistory(taskId: string, card: Element): Promise<void> {
  ...
  const result = await window.columbinaScheduler!.getHistory(taskId, 10);
...
function switchSection(section: string): void {
  ...
  const isColumbina = section === "columbina";
...
(function () {
  ...
  const KEY = "columbina.reranker.mode";
...
(function () {
  ...
  downloadBtn?.addEventListener("click", async () => {
    ...
    await window.system?.openExternal(
      "https://github.com/Playa-0v0/Columbina-Agent/blob/master/docs/local-models.md"
    );
  });
  ...
  mirrorGroup?.addEventListener("click", (e) => {
    ...
    localStorage.setItem("columbina.rag.mirror", value);
  });

  // Restore saved mirror on load
  const savedMirror = localStorage.getItem("columbina.rag.mirror") || "official";
  mirrorGroup?.querySelectorAll(".option-block").forEach((b) => {
    const v = b.getAttribute("data-value");
