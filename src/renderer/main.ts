...
if (!window.columbina) {
  (window as unknown as { columbina: unknown }).columbina = {
    minimize: () => {},
    hide: () => {},
    quit: () => {},
...
const manager = new Live2DManager({
  ...
  modelPath: resolveAsset("models/columbina/Columbina.model3.json"),
  onLoad: () => {
    console.log("[Columbina] Model loaded");
    ...
    interaction = new InteractionController(canvas, model, manager.getHitAreaDefs(), {
      onTrigger: (area) => {
        ...
        console.log("[Columbina] hit", area.name, "->", area.group + ":" + area.motionName);
      },
      onMiss: (area) =>
        console.warn("[Columbina] hit", area.name, "has no resolvable motion"),
    });
    ...
    clickThrough = new ClickThroughController(canvas, manager, {
      onInteractive: (interactive) => void window.columbina.setInteractive(interactive),
    });
    ...
    // the model to match.
    petZoomOff = window.columbina.onPetZoom((zoom) => manager.applyZoom(zoom));
    ...
    (window as unknown as { __columbina: unknown }).__columbina = {
      manager,
      interaction,
      focus,
      expressionReset,
  ...
  onError: (err) => {
    console.error("[Columbina] Failed to load model:", err);
  },
});
...
async function showDragOverlay(token: number): Promise<void> {
  const frame = await window.columbina.captureFrame();
...
function flushMove(): void {
  rafId = null;
  if (pendingPosition) {
    window.columbina.moveTo(pendingPosition.x, pendingPosition.y);
...
function finishDrag(): void {
  ...
  manager.resume();
  focus?.resume();
  window.columbina.setDragging(false);
  clickThrough?.resume();
}
...
canvas.addEventListener("pointerleave", () => {
  ...
  void window.columbina.setInteractive(false);
});
...
canvas.addEventListener("pointerdown", (e) => {
  ...
  focus?.pause(true);
  manager.pause();
  void window.columbina.setInteractive(true);
  window.columbina.setDragging(true);
  try {
...
canvas.addEventListener("pointerup", (e) => {
  ...
  if (outside) void window.columbina.setInteractive(false);
});
