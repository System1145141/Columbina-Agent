...
const roleGroupRight = document.getElementById("role-right") as HTMLElement | null;
const roleModelLeft = document.getElementById("role-model-left") as HTMLSelectElement | null;
...
const selectedModelId: Record<AgentRole, string | null> = {
  columbina: null,
  sandrone: null,
};

function getSelectedModelForRole(role: AgentRole): ModelEntry | null {
  const id = selectedModelId[role];
  if (!id || !currentModelConfig) return null;
  return currentModelConfig.models.find((m) => m.id === id) ?? null;
}

function populateModelSelect(select: HTMLSelectElement | null, models: ModelEntry[], selectedId: string | null): void {
  if (!select) return;
  select.innerHTML = "";
  // 占位选项
  ...
  for (const m of models) {
    ...
    opt.value = m.id;
    opt.textContent = m.nickname || m.model || m.id;
...
/** 切换当前选中的角色并更新 UI。 */
function switchRole(role: AgentRole): void {
  currentRole = role;
  if (roleToggle) roleToggle.checked = (role === "sandrone");
  if (roleGroupLeft) roleGroupLeft.classList.toggle("chat__role-group--active", role === "columbina");
  if (roleGroupRight) roleGroupRight.classList.toggle("chat__role-group--active", role === "sandrone");
  // 更新空态文本
  if (emptyTextEl) {
...
async function refreshModelConfig(): Promise<boolean> {
  try {
    ...
    console.warn("[Columbina Chat] model config unavailable:", err);
...
async function saveSession(): Promise<void> {
  ...
  try {
    ...
    console.warn("[Columbina Chat] saveSession 失败:", err);
  }
}
...
async function renderRailList(): Promise<void> {
  ...
  try {
    ...
    console.warn("[Columbina Chat] 侧栏加载会话列表失败:", err);
  }
...
chatRailNew?.addEventListener("click", async () => {
  ...
  try {
    ...
    console.warn("[Columbina Chat] 新建会话失败:", err);
  }
});
...
async function maybeMigrateLegacy(): Promise<void> {
  ...
  try {
    ...
    console.warn("[Columbina Chat] 旧 localStorage 迁移失败:", err);
  } finally {
...
async function bootstrap(): Promise<void> {
  if (!window.chatStore) {
    console.warn("[Columbina Chat] chatStore IPC 未就绪——可能是 preload 未加载");
    render();
    return;
  }
  ...
  if (roleToggle) {
    roleToggle.addEventListener("change", () => {
      const newRole: AgentRole = roleToggle.checked ? "sandrone" : "columbina";
      switchRole(newRole);
    });
  }
  // ── 模型选择下拉事件 ──────────────────────────────────
...
function render(): void {
  // 空态：当前会话还没有消息时（新建/全清）显示"期待与你聊天"占位
  // thinking 状态（agent 主动开场/流式回复中）也算有消息，胶囊应立即消失
...
function buildQuickPresets(): void {
  ...
  for (const preset of getQuickPresets()) {
    const btn = document.createElement("button");
    btn.type = "button";
...
function onPresetClick(preset: QuickPreset): void {
  if (preset.mode === "fill") {
    ...
    void triggerAgentGreeting();
  }
}

/**
 * 「和当前角色聊天」胶囊：让 agent 主动开口。
 * 注入隐藏种子消息触发 agent（不推入 messages 数组、不渲染），
 * 复用现有 AG-UI 流式回复机制。
 */
async function triggerAgentGreeting(): Promise<void> {
...
async function send(): Promise<void> {
  ...
  if (!currentSessionId) {
    console.warn("[Columbina Chat] 会话尚未初始化完成，已忽略此次发送");
    return;
  }
...
window.settings?.onPermissionApprovalRequest?.((req) => {
  console.log("[Columbina/Chat] permission approval request:", req.id, req.toolId);
