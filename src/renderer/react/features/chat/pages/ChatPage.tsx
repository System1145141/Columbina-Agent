...
function chatStore(): ChatStoreApi | undefined {
  // UI 移植（阶段 B）：优先走 preload 适配层（reactBridge），缺失时回退原生 window.chatStore。
  const bridge = (window as typeof window & { reactBridge?: { chatStore?: ChatStoreApi } })
    .reactBridge?.chatStore;
...
function aguiApi(): AguiApi | undefined {
  // UI 移植（阶段 B）：优先走 preload 适配层（reactBridge，含 run 参数翻译 + CUSTOM 事件名映射）。
  const bridge = (window as typeof window & { reactBridge?: { agui?: AguiApi } })
    .reactBridge?.agui;
  if (bridge) return bridge;
