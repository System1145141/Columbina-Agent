...
export function WindowControls({ onMinimize, onMaximize, onClose }: WindowControlsProps) {
  return (
    <div className="cy-window-controls">
      <button type="button" className="cy-winbtn cy-winbtn--minimize" onClick={onMinimize} aria-label={t("reactChat.minimize")}>
        <svg width="10" height="2" viewBox="0 0 10 2" aria-hidden="true">
          <rect width="10" height="2" rx="1" fill="currentColor" />
        </svg>
      </button>
      <button type="button" className="cy-winbtn cy-winbtn--maximize" onClick={onMaximize} aria-label={t("reactChat.maximizeRestore")}>
        <svg width="10" height="10" viewBox="0 0 10 10" aria-hidden="true">
          <rect x="0.75" y="0.75" width="8.5" height="8.5" rx="1.5" fill="none" stroke="currentColor" strokeWidth="1.2" />
        </svg>
      </button>
      <button type="button" className="cy-winbtn cy-winbtn--close" onClick={onClose} aria-label={t("reactChat.closeChatWindow")}>
        <svg width="10" height="10" viewBox="0 0 10 10" aria-hidden="true">
