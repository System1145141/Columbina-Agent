...
function parseProgressFile(content: string): LearnProgress {
  ...
  try {
    ...
    if (fm.topics && typeof fm.topics === "object") {
      for (const [key, val] of Object.entries(fm.topics as Record<string, unknown>)) {
    ...
    return {
      schemaVersion: 1,
      currentTopic: typeof fm.currentTopic === "string" ? fm.currentTopic : undefined,
      currentSection:
        typeof fm.currentSection === "string" ? fm.currentSection : undefined,
      topics,
      nextStep: typeof fm.nextStep === "string" ? fm.nextStep : undefined,
      updatedAt:
