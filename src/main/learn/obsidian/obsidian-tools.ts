/**
 * Obsidian Tools — 将 ObsidianWorkspaceService 封装为 Columbina ToolRegistry 工具。
 *
 * 6 个工具：
 * - obsidian_list_files
 * - obsidian_search
 ...
 * - obsidian_open_note
 *
 * 仅在会话 mode === "learn" 且绑定 workspaceRoot（Vault）时注册。
 * 风险等级按 Columbina 惯例：只读工具 risk=fs-read 自动执行；
 * obsidian_edit 为写操作 risk=fs-write + needsConfirm；obsidian_open_note 为外部副作用 risk=network。
 */
