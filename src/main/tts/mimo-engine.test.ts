...
describe("mimo-engine synthesize", () => {
  ...
  it("sends Xiaomi MiMo voiceclone payload with the selected Columbina voice sample", async () => {
    const voiceAudioPath = writeTempVoiceFile("columbina.mp3", "columbina voice sample");
    ...
    expect(JSON.parse(String(request.body))).toEqual({
      ...
      audio: {
        format: "wav",
        voice: "data:audio/mpeg;base64,Y29sdW1iaW5hIHZvaWNlIHNhbXBsZQ==",
      },
    });
  });
  ...
  it("omits empty style prompts", async () => {
    const voiceAudioPath = writeTempVoiceFile("columbina.wav", "wav sample");
