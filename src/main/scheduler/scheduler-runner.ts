...
import { ColumbinaAgent, type ColumbinaRunOptions } from "../orchestrator/columbina-agent";
...
interface RunnerDeps {
  buildOptions: (task: ScheduledTask) => Promise<ColumbinaRunOptions>;
...
export function createSchedulerRunner(deps: RunnerDeps) {
  async function runScheduledTask(task: ScheduledTask, _scheduledFireAt: Date, manual: boolean): Promise<ScheduledRunResult> {
    ...
    try {
      ...
      const agent = new ColumbinaAgent({ threadId: `scheduler-${task.id}`, description: `Scheduled task: ${task.title}` });
