...
//   - 共享密钥 header：X-Columbina-Channel-Secret（启动时自动生成 32 字节 hex）
...
function checkSecret(req: http.IncomingMessage, secret: string): boolean {
  ...
  const got = req.headers["x-columbina-channel-secret"];
...
/** 给 runtime 计算一个 HMAC（用作 X-Columbina-Channel-Secret 的 payload 签名场景，备用） */
