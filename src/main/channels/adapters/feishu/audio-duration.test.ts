...
describe("audio-duration (零依赖方案: ffprobe + MP3 header + 文件大小估算)", () => {
  ...
  it("空文件 → 返回 undefined 不崩", async () => {
    const tmp = path.join(os.tmpdir(), `columbina-empty-${Date.now()}.mp3`);
  ...
  it("纯文本文件 → 走兜底估算 (返回 ≥500ms)", async () => {
    const tmp = path.join(os.tmpdir(), `columbina-text-${Date.now()}.mp3`);
  ...
  it("合法 CBR mp3 → MP3 header 解析给到正毫秒数", async () => {
    const tmp = path.join(os.tmpdir(), `columbina-cbr-${Date.now()}.mp3`);
  ...
  it("合法 CBR mp3 (小文件, 仅 1 帧 ~26ms) → 返回合理值", async () => {
    const tmp = path.join(os.tmpdir(), `columbina-tiny-${Date.now()}.mp3`);
    writeCbrMp3(tmp, 1);
    try {
      const r = await getAudioDurationMs(tmp);
      expect(r).toBeDefined();
