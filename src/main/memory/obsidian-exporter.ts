...
export async function exportMemoryToObsidianVault(outputDir: string): Promise<ExportResult> {
  try {
    ...
    console.info(`[obsidian-export] exported ${writtenFiles.length} files to ${outputDir}`);
    ...
    console.error(`[obsidian-export] failed: ${msg}`);
...
export function notifyMemoryChanged(): void {
  ...
  syncTimer = setTimeout(() => {
    ...
    void syncToBoundVault().then((r) => {
      if (r.ok) {
        console.info(`[obsidian-sync] auto-synced ${r.fileCount} files to ${r.vaultPath}`);
      } else if (!r.skipped) {
        console.warn(`[obsidian-sync] auto-sync failed: ${r.error}`);
      }
    });
  }, SYNC_DEBOUNCE_MS);
}
