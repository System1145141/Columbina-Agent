...
async function processPending(): Promise<void> {
  ...
  if (changedCount > 0) {
    console.info(`[obsidian-import] imported ${changedCount} changed memory(ies) from vault`);
  }
}
...
export function startVaultWatcher(vaultPath: string): void {
  ...
  try {
    ...
    console.info(`[obsidian-import] watcher started on ${l2Dir}`);
  } catch (err) {
    console.warn(
...
export function stopVaultWatcher(): void {
  ...
  pendingFiles.clear();
  if (watchedVaultPath) {
    console.info(`[obsidian-import] watcher stopped`);
    watchedVaultPath = null;
  }
}
