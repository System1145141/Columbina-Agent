...
function migrateProviderRenames(
  ...
  for (const [key, value] of Object.entries(perProvider)) {
    ...
    if (next[newKey]) {
      ...
      console.log("[Columbina] provider rename: drop legacy", key, "→ kept", newKey);
      continue;
    }
    if (newKey !== key) {
      console.log("[Columbina] provider rename:", key, "→", newKey);
    }
    next[newKey] = value;
  }
...
function loadModelSettings(): ModelSettings {
  try {
    ...
    console.error("[Columbina] load settings failed:", err);
...
async function syncVolcanoSearchMcp(settings: GeneralSettings): Promise<void> {
  ...
  if (minimaxEnable && !minimaxExists) {
    console.log("[Columbina] 注册 MiniMax 搜索 MCP Server...");
    try {
      ...
      if (result.ok) {
        console.log("[Columbina] MiniMax 搜索 MCP 注册成功，工具:", result.toolIds?.join(", "));
      } else {
        console.error("[Columbina] MiniMax 搜索 MCP 注册失败:", result.error);
      }
    } catch (err) {
      console.error("[Columbina] MiniMax 搜索 MCP 注册异常:", err);
    }
  } else if (!minimaxEnable && minimaxExists) {
    console.log("[Columbina] 移除 MiniMax 搜索 MCP Server...");
    try { await removeMcpServer(MINIMAX_SEARCH_MCP_ID); } catch (err) { console.error("[Columbina] MiniMax 搜索 MCP 移除异常:", err); }
  } else if (minimaxEnable && minimaxExists) {
    console.log("[Columbina] MiniMax 搜索 key 变化，重新注册 MCP Server...");
    try {
    ...
    } catch (err) { console.error("[Columbina] MiniMax 搜索 MCP 重新注册异常:", err); }
  }
}
...
function loadStickerSettings(): Record<string, boolean> {
  ...
  try {
    ...
    console.error("[Columbina] load sticker settings failed:", err);
  }
...
/** GeneralSettings.language 映射到 prompt 目录语言代码。 */
function langToPromptDir(lang?: string): string {
  if (!lang || lang === "zh-CN") return "cn";
...
export function loadPromptFile(filename: string, identityId?: string, lang = "cn"): string {
  try {
    ...
    // 1) 角色语言目录：prompts/{identityId}/{promptDir}/{filename}
    if (identityId) {
      const roleLangPath = path.join(app.getAppPath(), "prompts", identityId, promptDir, filename);
      if (fs.existsSync(roleLangPath)) return fs.readFileSync(roleLangPath, "utf8").trim();
    }

    // 2) 系统语言目录：prompts/system/{promptDir}/{filename}
    const sysLangPath = path.join(app.getAppPath(), "prompts", "system", promptDir, filename);
    if (fs.existsSync(sysLangPath)) return fs.readFileSync(sysLangPath, "utf8").trim();

    // 3) 角色扁平目录（旧结构兼容）：prompts/{identityId}/{filename}
    if (identityId) {
      const roleFlatPath = path.join(app.getAppPath(), "prompts", identityId, filename);
      if (fs.existsSync(roleFlatPath)) return fs.readFileSync(roleFlatPath, "utf8").trim();
    }

    // 4) 系统扁平目录（旧结构兼容）：prompts/system/{filename}
    const sysFlatPath = path.join(app.getAppPath(), "prompts", "system", filename);
    if (fs.existsSync(sysFlatPath)) return fs.readFileSync(sysFlatPath, "utf8").trim();

    // 5) 根级回退（旧结构兼容）：prompts/{filename}
    const rootPath = path.join(app.getAppPath(), "prompts", filename);
    if (fs.existsSync(rootPath)) return fs.readFileSync(rootPath, "utf8").trim();
...
function buildSystemPrompt(styleFile: string, identityId?: string, lang = "cn"): string {
  const parts: string[] = [];

  // styleFile 以 "talk" 开头时走纯聊天模式：用 talk_system.md 替换 system.md（不调工具）
  const isTalkMode = styleFile.startsWith("talk");
  const system = loadPromptFile(isTalkMode ? "talk_system.md" : "system.md", identityId, lang);
  ...
  if (!isTalkMode) {
    const style = loadPromptFile("styles/" + styleFile, identityId, lang);
...
function resolveSlashActivation<T extends { role: string; content: string }>(messages: T[]): string {
  ...
  if (skill && skill.enabled) {
    ...
    if (body !== null) {
      console.log("[Columbina] /命令激活 skill:", parsed.skillId);
...
async function observeRuntimeState(
  ...
  enqueueLLMTask("心情观察器", async () => {
    ...
    console.warn("[Columbina] observe runtime failed; keeping current feeling:", err);
  });
  // 标注未使用的参数，避免 lint 警告
  void latestUserText;
}
...
async function requestModelReply(inputMessages: unknown, styleFile = "01_default.md"): Promise<ChatReplyPayload> {
  ...
  try {
    ...
    console.warn("[Columbina] memory injection failed:", err);
  }
  ...
  if (sceneProvider && sceneEmbeddingIndex) {
    try {
      toneInjection = await buildToneInjection(latestUserText, messages, sceneProvider, sceneEmbeddingIndex, promptLang);
    } catch (err) {
      console.error("[Columbina] tone injection failed:", err);
    }
  }
  ...
  const systemContent =
    ...
    buildSystemPrompt(styleFile, undefined, promptLang) +
    (skillCatalog ? "\n\n---\n\n" + skillCatalog : "") +
    skillActivation +
  ...
  try {
    ...
    if (fcResult.toolResults.length > 0) {
      console.log("[Columbina] Function Calling 使用了 " + fcResult.toolResults.length + " 个工具:",
...
function broadcastRuntimeStateChanged(): void {
  console.log("[Columbina] broadcasting runtime state:", JSON.stringify(runtimeState));
...
function createWindow(): void {
  ...
  setCallSettings(
    ...
    async (userText: string) => {
      ...
      const promptLang = langToPromptDir(loadGeneralSettings().language);
      ...
      // ④ 通话专用人设 prompt（TODO: call handler 传入 identityId）
      const callIdentityId: string | undefined = undefined;
      const phoneParts: string[] = [];
      const phoneSystem = loadPromptFile("phone_system.md", callIdentityId);
      if (phoneSystem) phoneParts.push(phoneSystem);
      const phoneIdentity = loadPromptFile("phone_identity.md", callIdentityId);
      if (phoneIdentity) phoneParts.push(phoneIdentity);
      const soul = loadPromptFile("soul.md", callIdentityId);
      if (soul) phoneParts.push(soul);
      const canon = loadPromptFile("canon_quotes.md", callIdentityId);
      ...
      const sceneProvider = getSceneEmbeddingProvider();
      if (sceneProvider && sceneEmbeddingIndex) {
        try { toneInjection = await buildToneInjection(userText, messages, sceneProvider, sceneEmbeddingIndex, promptLang); } catch { /* ignore */ }
      }
...
ipcMain.handle(IPC.WINDOW_CAPTURE_FRAME, async () => {
  ...
  try {
    ...
    console.error("[Columbina] captureFrame failed:", err);
    return null;
  }
});
...
ipcMain.handle("debug:screenshot", async () => {
  ...
  const outPath = path.join(app.getPath("temp"), "columbina-screenshot.png");
...
ipcMain.handle(IPC.SETTINGS_TEST_CONNECTION, async (_event, cfg: { provider: string; baseUrl: string; model: string; apiKey: string }) => {
  ...
  console.log("[Columbina] test connection: provider=" + cfg.provider + " transport=" + adapter.transport + " model=" + cfg.model);
  const result = await adapter.testConnection(cfg);
  console.log("[Columbina] test connection result:", JSON.stringify(result));
  return result;
});
...
ipcMain.handle(IPC.SETTINGS_TEST_VISION, async (_event, cfg: { baseUrl: string; apiKey: string; model: string }) => {
  ...
  console.log("[Columbina] test vision: model=" + cfg.model + " url=" + cfg.baseUrl);
  try {
...
ipcMain.handle(IPC.EMBEDDING_SET_MODEL, async (_event, modelKey: string) => {
  console.log("[Columbina] embedding model switch requested:", modelKey);
  try {
    ...
    console.error("[Columbina] embedding model switch failed:", message);
...
ipcMain.handle(IPC.RERANKER_SET_MODE, async (_event, mode: "light" | "standard" | "none") => {
  ...
  console.log("[Columbina] reranker mode switched to", mode);
  return true;
});
...
app.whenReady().then(async () => {
  ...
  if (removed.length > 0) {
    console.log("[Columbina] 已清理遗留的已下架内置 MCP:", removed.join(", "));
  }

  void syncPlaywrightMcp(initialSettings).catch((e) =>
    console.error("[Columbina] playwright MCP sync failed:", e)
  );
  ...
  setDispatcherBuildAndRunAgent(async (msg, sessionId, priorMessages) => {
    ...
    // 把 IncomingMessage 转成 AguiRunInput，调 ColumbinaAgent
    ...
    const agent = new ColumbinaAgent({ threadId, description: `bot:${msg.channel}:${msg.senderId}` });
  ...
  //   渲染端收到 columbina.todos 后渲染左上角进度面板
  loadTodos();
  onTodosChange((state) => {
    for (const win of BrowserWindow.getAllWindows()) {
      if (win.isDestroyed()) continue;
      try {
        win.webContents.send(IPC.AGUI_EVENT, {
          type: "CUSTOM",
          name: "columbina.todos",
          value: state,
        });
      } catch (e) {
        console.warn("[Columbina] todos 广播失败:", e);
      }
    }
  });
  ...
  const schedulerRunner = createSchedulerRunner({
    buildOptions: async (task: ScheduledTask) => {
      ...
      const systemContent =
        (environmentContext ? environmentContext + "\n\n" : "") +
        (alwaysOnContext ? alwaysOnContext + "\n\n" : "") +
        buildSystemPrompt("01_default.md", "columbina") +
        (skillCatalog ? "\n\n---\n\n" + skillCatalog : "");
      return {
  ...
  // AG-UI 事件流桥：渲染进程 invoke(AGUI_RUN) → ColumbinaAgent 跑 FC 循环 → 事件透传
  ...
  const buildOptionsDeps: BuildOptionsDeps = {
    ...
    promptLang: langToPromptDir(loadGeneralSettings().language),
    buildToneInjection: (async (userText, messages, provider, index, lang) =>
      buildToneInjection(userText, messages as any, provider as any, index as any, lang)) as BuildOptionsDeps["buildToneInjection"],
