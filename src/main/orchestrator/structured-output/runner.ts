...
export async function runStructuredOutput<T, TRequest>(
  ...
  // 统一模型请求超时（移植适配）：
  // Cyrene 从 timeout-manager / config/model-timeout 读取用户配置；Columbina 无这两个模块，
  // 使用与原模块默认行为一致的本地常量：
  //   per-attempt = 60s；总预算 = per-attempt × maxAttempts；
  //   minimumRemainingBudgetMs 保留 profile 原值（原模块默认 -1 即不覆盖）。
  const PER_ATTEMPT_TIMEOUT_MS = 60_000;
  const perAttempt = PER_ATTEMPT_TIMEOUT_MS;
  const totalBudget = perAttempt * Math.max(1, policy.maxAttempts);
  clonePolicyIfNeeded();
