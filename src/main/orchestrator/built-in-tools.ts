...
async function executeFetchUrl(args: Record<string, unknown>): Promise<string> {
  ...
  try {
    const resp = await fetch(url, {
      signal: ac.signal,
      headers: {
        "User-Agent": "Mozilla/5.0 (Columbina Agent) Chrome/120 Safari/537.36",
...
import { summonKuuhenki, setKuuhenkiSettings } from "./kuuhenki";

export { setKuuhenkiSettings };
// 把重任务委托给 Kuuhenki（月灵）独立 FC 循环执行，Kuuhenki 有自己的 conversation（用完即弃）。
// 执行完只返回结构化摘要给主 agent，不被重工具的过程数据（skill 正文、XML 文件等）污染。
toolRegistry.register({
  id: "summon_kuuhenki",
  name: "召唤月灵",
  description:
    "召唤 Kuuhenki（月灵）—— 你的忠实眷属，把需要多步工具调用的子任务委托给它独立执行。\n" +
    "Kuuhenki 有自己的上下文（不占用主对话空间），执行完返回结构化摘要。\n\n" +
    "何时用：\n" +
    "- 任务需要 ≥2 步工具调用且中间结果不需要用户确认\n" +
    "- 涉及大量中间数据（如读取 skill 文档 + 生成文件），不想让中间内容占用主对话上下文\n" +
    "- 例：「用 xlsx skill 生成带公式的 Excel」→ Kuuhenki 内部读 create.md + format.md + 写 XML，主对话只看到最终摘要\n\n" +
    "不要用于：\n" +
    "- 单步操作（直接调对应工具即可）\n" +
    "- 需要跟用户交互的任务（Kuuhenki 不能弹卡片）\n" +
    "- 简单表格生成（直接用 write_excel）\n\n" +
    "参数：task（子任务的完整描述，Kuuhenki 会独立理解并执行）。" ,
  enabled: true,
  risk: "safe",
  inputSchema: {
    type: "object",
    properties: {
      task: { type: "string", description: "子任务的完整描述。要足够详细让 Kuuhenki 能独立执行，如「读取 test20.txt 的商品价格，查汇率换算成人民币，用 write_excel 生成深色风格 Excel 存到桌面 test 文件夹」" },
    },
    required: ["task"],
  },
  execute: async (args) => {
    ...
    console.log(LOG_PREFIX, "summon_kuuhenki:", task.slice(0, 100));
    const result = await summonKuuhenki(task);

    if (result.status === "success") {
      let output = `[summon_kuuhenki] Kuuhenki 执行成功：${result.summary}`;
    ...
    let output = `[summon_kuuhenki] Kuuhenki 执行失败：${result.summary}`;
...
console.log(LOG_PREFIX, "已注册：fetch_url / run_shell / install_mcp_server / weather / web_search / ask_user_choice / summon_kuuhenki");
