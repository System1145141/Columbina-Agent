...
/** 从 prompts/system/{lang}/tone-rules.md 加载语气规则，文件不存在时用内置默认值。 */
function loadToneRules(lang = "cn"): string {
  try {
    const promptDir = langToPromptDir(lang);
    // 优先从 system/{lang} 读取
    const sysLangPath = path.join(app.getAppPath(), "prompts", "system", promptDir, "tone-rules.md");
    if (fs.existsSync(sysLangPath)) {
      const content = fs.readFileSync(sysLangPath, "utf8").trim();
      const body = content.startsWith("---") ? content.replace(/^---[\s\S]*?---\n?/, "").trim() : content;
      if (body.length > 0) return "## 语气规则\n\n" + body;
    }
    // 回退到 prompts/tone-rules.md（旧结构兼容）
    const oldPath = path.join(app.getAppPath(), "prompts", "tone-rules.md");
...
function loadSceneSamples(scene: SceneId): string {
  if (!scene) return "";
  try {
    const skillDir = path.join(app.getAppPath(), "skills", "columbina-original-voice", "references");
 ...
 * @param lang 语言代码（zh-CN/cn/en/jp/ko），用于加载对应语言的 tone-rules.md
...
export async function buildToneInjection(
  ...
  const toneRules = loadToneRules(lang);
