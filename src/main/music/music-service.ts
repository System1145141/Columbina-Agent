...
export class MusicService {
  ...
  async start(): Promise<void> {
    ...
    try {
      // 外部依赖降级：resources/music-vendor（uv + cloud-music-mcp）缺失时，
      // 进入 degraded 而非 failed——不阻塞应用启动与其它功能，
      // 登录/搜索等操作返回 E_MUSIC_DEPENDENCY_MISSING（"未安装音乐依赖"）提示。
      const vendorExists = await fs.stat(this.paths.vendorDir).then(() => true).catch(() => false);
      if (!vendorExists) {
        console.warn("[music] vendor dir missing, degrading:", this.paths.vendorDir);
        this.backendState = "degraded";
        this.emitBackendChange("degraded");
        return;
      }
  ...
  async presentTracks(params: {
    ...
    const cardRef = `columbina:music:${setId}:${trackIds.join(":")}`;
    return { cardRef };
  }
  ...
  private requireReady(): void {
    if (this.backendState === "degraded") {
      throw new MusicInputError("E_MUSIC_DEPENDENCY_MISSING", "未安装音乐依赖（缺少 resources/music-vendor）");
    }
