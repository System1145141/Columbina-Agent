...
// start() 会先校验 vendor 目录是否存在（缺失 → degraded），
// 因此这里在 beforeEach 建一个真实临时 vendor 目录，指向它的 MusicService 才能走到 "ready"。
let vendorDir = "";
beforeEach(async () => {
  vendorDir = await fs.mkdtemp(path.join(os.tmpdir(), "music-vendor-"));
...
/** 指向真实临时 vendor 目录的 paths（start() 不会降级）。 */
function readyPaths(): { vendorDir: string; runtimeDir: string; accountPath: string; resourceBaseDir: string } {
...
describe("MusicService", () => {
  it("degrades gracefully when the vendor dir is missing", async () => {
    const missing = new MusicService({ ...PATHS, vendorDir: path.join(os.tmpdir(), "definitely-missing-vendor-" + Date.now()) });
    await missing.start();
    expect(missing.getBackendState()).toBe("degraded");
    // 登录/搜索返回"未安装音乐依赖"提示（code=E_MUSIC_DEPENDENCY_MISSING），而不是 E_BACKEND_NOT_READY
    await expect(missing.searchTracks("稻香", "c1")).rejects.toMatchObject({ code: "E_MUSIC_DEPENDENCY_MISSING" });
    await expect(missing.beginLogin()).rejects.toMatchObject({ code: "E_MUSIC_DEPENDENCY_MISSING" });
  });
  ...
  it("searchTracks returns a set after start", async () => {
    ...
    const s = new MusicService(readyPaths());
    await s.start();
