    ...
    if (!newProvider) {
      try {
        ...
        if (detail.existingProjectDir) {
          ...
          console.error(
            `[Columbina] embedding model "${modelKey}" project directory exists but is incomplete.\n` +
          ...
          console.error(
            `[Columbina] embedding model "${modelKey}" not detected anywhere.\n` +
        ...
        console.error("[Columbina] model diagnostic log failed:", diagErr);
      }
