...
- 💬 **AI 对话** — React 聊天窗口（antd-x 流式 Markdown / 思维链折叠 / 工具调用行 / 音乐卡片 / 图片附件），双角色切换，多会话历史，4 语 i18n，深色主题适配
- 🧠 **记忆引擎** — L0/L1/L2 + Worldbook + L2-DMAE 激活状态机 + Obsidian 双向同步 + 启动向量对齐
- 🗂️ **文档知识** — RAG 后台文档索引（拖入目录自动切分/向量化/检索）
- 🎵 **音乐功能** — 网易云搜索/播放/歌单 + 二维码登录（可选，需 uv + cloud-music-mcp）
- 🌙 **主动聊天** — 无人交互时按场景主动开口，投递到桌面或微信/飞书
- 📚 **学习陪伴（Learn）** — 绑定 Obsidian Vault，Agent 读写笔记并追踪学习进度
- 🧠 **CITA 上下文认知** — 引用消解与上下文聚焦，提升工作模式下的多轮理解
- 💬 **社交上下文** — 记住"上次没聊完的事"，跨会话衔接对话
- 🔊 **语音通话** — TTS（含 Mossland 引擎）+ ASR，解放双手
- 🛠 **工具生态** — 文档生成、联网搜索、文件操作、出行、邮件等
- 📱 **多平台接入** — 飞书、微信 iLink
